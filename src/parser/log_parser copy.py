...
class ParsedTransaction:
    ...
    amount_original: Optional[float] = None  # Raw amount in original currency
    currency: Optional[str] = None  # Original currency
    amount: Optional[float] = None  # Amount converted to GBP
...
class TransactionLogParser:
    ...
    def __init__(self):
        ...
        self.exchange_rates = {
            ...
            '€': 0.87,  # 1 EUR = 0.87 GBP
            '£': 1.0,   # 1 GBP = 1.0 GBP
            'USD': 0.75,  # 1 USD = 0.75 GBP
            'EUR': 0.87,  # 1 EUR = 0.87 GBP
            'GBP': 1.0,   # 1 GBP = 1.0 GBP
    ...
    def _parse_format_2(self, log_entry: str, transaction: ParsedTransaction) -> bool:
        ...
        try:
            ...
            if amount_match:
                ...
                transaction.amount_original = float(amount_match.group(2))
    ...
    def _validate_and_clean(self, transaction: ParsedTransaction):
        ...
        # Set currency if amount_original exists but currency doesn't
        if transaction.amount_original is not None and not transaction.currency:
            # Try to infer currency from location
            inferred_currency = self._infer_currency_from_location(transaction.location)
            transaction.currency = inferred_currency
            transaction.parse_errors.append(f"Inferred currency '{inferred_currency}' from location")
        
        # Convert currency to GBP and standardize amount
        if transaction.amount_original is not None and transaction.currency:
            transaction.amount = self._convert_to_gbp(transaction.amount_original, transaction.currency)
    ...
    def _infer_currency_from_location(self, location: str) -> str:
        """Infer currency based on location."""
        if not location:
            return 'GBP'  # Default to GBP if no location
        
        location_lower = location.lower()
        
        # UK locations - GBP
        uk_locations = ['london', 'glasgow', 'birmingham', 'liverpool', 'cardiff', 'leeds', 'manchester']
        if any(uk_loc in location_lower for uk_loc in uk_locations):
            return 'GBP'
        
        # US locations - USD (if any US cities were in the data)
        ...
        # European locations - EUR (if any EU cities were in the data)
        eu_locations = ['paris', 'berlin', 'madrid', 'rome', 'amsterdam', 'brussels']
        if any(eu_loc in location_lower for eu_loc in eu_locations):
            return 'EUR'
        
        # Default to GBP for unknown locations
        return 'GBP'
    
    def get_currency_conversion_stats(self, df_parsed: pd.DataFrame) -> Dict[str, Any]:
        """Generate statistics about currency conversions."""
        if df_parsed.empty:
            return {}
        
        # Count original currencies
        currency_counts = df_parsed['original_currency'].value_counts().to_dict()
        
        # Calculate conversion statistics
        conversion_stats = {
            'total_transactions': len(df_parsed),
            'transactions_with_amount': len(df_parsed[df_parsed['amount'].notna()]),
            'currency_distribution': currency_counts,
            'exchange_rates_used': self.exchange_rates,
            'conversion_summary': {
                'gbp_transactions': currency_counts.get('GBP', 0),
                'usd_transactions': currency_counts.get('USD', 0),
                'eur_transactions': currency_counts.get('EUR', 0),
                'unknown_currency': currency_counts.get('unknown', 0)
            }
        }
        
        # Calculate total amounts in each original currency
        if 'amount' in df_parsed.columns and 'original_currency' in df_parsed.columns:
            currency_totals = {}
            for currency in ['GBP', 'USD', 'EUR']:
                mask = df_parsed['original_currency'] == currency
                total = df_parsed.loc[mask, 'amount'].sum()
                currency_totals[currency] = total
            
            conversion_stats['total_amounts_by_currency'] = currency_totals
    ...
    def parse_dataset(self, file_path: str) -> Tuple[pd.DataFrame, Dict[str, Any]]:
        ...
        # Add currency conversion statistics
        parsing_stats['currency_conversion'] = self.get_currency_conversion_stats(df_parsed)
